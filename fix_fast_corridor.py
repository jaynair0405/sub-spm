#!/usr/bin/env python3
"""
Fix DNFASTLOCALS.csv by filling in missing inter-station distances from DNSLOWLOCALS.csv
"""

import csv

# Read slow corridor data
with open('DNSLOWLOCALS.csv', 'r') as f:
    reader = csv.reader(f)
...
with open('DNFASTLOCALS.csv', 'r') as f:
    reader = csv.reader(f)
    ...
    for i, fast_station in enumerate(fast_stations):
        ...
        if i == 0:
            ...
            if prev_fast_station and prev_fast_station in slow_idx and fast_station in slow_idx:
                ...
                # e.g., for BY→PR: sum CHG column + CRD column + PR column
                total_distance = sum(slow_isds[start_idx+1:end_idx+1])
                corrected_isds.append(total_distance)
                print(f"  {prev_fast_station}→{fast_station}: {total_distance:.0f}m (was empty)")
            else:
                corrected_isds.append(0.0)
                if prev_fast_station:
                    print(f"  {prev_fast_station}→{fast_station}: NOT FOUND in slow corridor")

    corrected_records.append([record_num] + corrected_isds)

# Write corrected file
with open('DNFASTLOCALS_corrected.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
print("\nCorrected file written to DNFASTLOCALS_corrected.csv")
